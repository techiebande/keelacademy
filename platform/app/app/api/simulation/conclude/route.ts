...
import { concludeSimulation } from "@/lib/simulation";
...
export async function POST(req: Request) {
  ...
  if (!body.simulation_id) {
    return NextResponse.json(
      { error: "missing_fields", message: "simulation_id is required" },
      { status: 422 },
    );
  }

  const res = await concludeSimulation({
