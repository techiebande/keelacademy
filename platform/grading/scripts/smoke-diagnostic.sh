#!/usr/bin/env bash
# smoke-diagnostic.sh — S4.1 deterministic proof: Day-Zero commitment declarations
# and foundations diagnostic placement engine with automatic unit unlock routing,
# spine event emission, and spoof immunity.
#
# Scratch postgres:16-alpine (0001..0010) seeded with:
# - alice: student with email alice@keel.test
# - bob:   student with email bob@keel.test
# - carol: student with email carol@keel.test
#
# Runs:
# - fake OpenAI judge upstream
# - proxy/server.py
# - practice/server.py
# - smoke-diagnostic-checks.py
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCHEMA_DIR="$SCRIPT_DIR/../schema"
IMAGE="postgres:16-alpine"
CONTAINER="keel-diag-smoke-$$"
DB_USER="smoke"
DB_NAME="grading"
...
APP_TOKEN="smoke-diagnostic-token-$$"
TRACE_LOG="$(mktemp /tmp/keel-diag-trace.XXXXXX.jsonl)"
SERVER_LOG="$(mktemp /tmp/keel-diag-servers.XXXXXX.log)"
NOW_FILE="$(mktemp /tmp/keel-diag-now.XXXXXX.txt)"
...
cleanup() {
    ...
    rm -f "$TRACE_LOG" "$SERVER_LOG" "$NOW_FILE" /tmp/keel-diag-* 2>/dev/null || true
...
echo "== applying schema 0001..0010 =="
for m in 0001_init 0002_intake 0003_budgets 0004_enrollments 0005_rebates 0006_gates 0007_practice 0008_retrieval 0009_concierge 0010_diagnostic; do
    "$DOCKER" exec -i "$CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" -v ON_ERROR_STOP=1 < "$SCHEMA_DIR/$m.sql" >/dev/null
done

echo "== seeding alice, bob, carol students =="
...
echo "== running diagnostic placement smoke checks =="
export KEEL_PRACTICE_URL="http://127.0.0.1:$PRACTICE_PORT"
python3 "$SCRIPT_DIR/smoke-diagnostic-checks.py"

echo "== ALL SMOKE DIAGNOSTIC CHECKS PASSED =="
