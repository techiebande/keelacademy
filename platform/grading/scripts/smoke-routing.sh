#!/usr/bin/env bash
# smoke-routing.sh — S3.4 deterministic proof: adaptive practice routing rules,
# derived from attempt history at read time under a deterministic clock
# (KEEL_PRACTICE_NOW_FILE, no sleeps), fast pass skips worked example,
# failure routes through worked example scaffold with deterministic deep links.
#
...
# - smoke-routing-checks.py
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCHEMA_DIR="$SCRIPT_DIR/../schema"
IMAGE="postgres:16-alpine"
CONTAINER="keel-routing-smoke-$$"
DB_USER="smoke"
DB_NAME="grading"
...
APP_TOKEN="smoke-routing-token-$$"
TRACE_LOG="$(mktemp /tmp/keel-routing-trace.XXXXXX.jsonl)"
SERVER_LOG="$(mktemp /tmp/keel-routing-servers.XXXXXX.log)"
NOW_FILE="$(mktemp /tmp/keel-routing-now.XXXXXX.txt)"
...
cleanup() {
    ...
    rm -f "$TRACE_LOG" "$SERVER_LOG" "$NOW_FILE" /tmp/keel-routing-* 2>/dev/null || true
...
echo "== running adaptive routing checks =="
export KEEL_PRACTICE_URL="http://127.0.0.1:$PRACTICE_PORT"
export KEEL_FAKE_URL="http://127.0.0.1:$FAKE_PORT"
export KEEL_ROUTING_NOW_FILE="$NOW_FILE"
python3 "$SCRIPT_DIR/smoke-routing-checks.py"

echo "== ALL SMOKE ROUTING CHECKS PASSED =="
