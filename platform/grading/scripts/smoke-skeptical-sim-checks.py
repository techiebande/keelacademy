...
def main() -> int:
    ...
    st, data = http_request("POST", "/simulation/turn", {
        ...
        "student_id": 101,
        "message": "We evaluated our case triage system against a golden set of 500 labeled historical cases, achieving 94.2% precision and 92.8% recall on rulebook clause verification.",
    })
    ...
    st, data = http_request("POST", "/simulation/turn", {
        "simulation_id": sim_tech_101_id,
        "student_id": 101,
        "message": "Our average cost per transaction is $0.04 using an aggressive prompt cache and a tiered model router where 80% of volume runs through Claude Haiku 4.5 at 320ms p99 latency, cascading to Sonnet only for high-complexity cases.",
    })
    last_persona_msg = data["turns"][-1]["content"]
    ...
    st, data = http_request("POST", "/simulation/turn", {
        ...
        "student_id": 101,
        "message": "This deployment saves Lantern Home $420,000 net annually by eliminating 1.8 hours of manual document review per case across 3,000 monthly transactions.",
    })
    last_persona_msg = data["turns"][-1]["content"]
    ...
    st, data = http_request("POST", "/simulation/turn", {
        "simulation_id": sim_biz_101_id,
        "student_id": 101,
        "message": "When an ambiguous $50,000 damage claim is encountered, the system never auto-denies or auto-approves credit; it routes the complete draft dossier with highlighted rulebook clauses directly to a senior operations specialist for human signoff.",
    })
    last_persona_msg = data["turns"][-1]["content"]
