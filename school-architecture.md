...
|---|---|
| Teaches you the concept in the first place | School-authored chapters that build the thing in front of the reader, faded assignments, and spaced recall (§2) + concierge teach-mode for unlimited questions (§5) |
...
**Unit pages.** Each of the 104 sub-modules becomes a **unit page**: a chapter, an assignment, and the platform's apparatus after them. (Granularity note: `curriculum.md` lists fine syllabus items like 1.1.1; `content/curriculum/phases.yaml` enrolls coarser modules like 1.1. Unlocks and prereqs may name either level.) The design and its evidence are in `docs/lesson-design.md`; the writing guide is `content/WRITING.md`.

- **The chapter** (`lesson.md`, or several chapter files for a long unit) is a school-authored piece of writing that teaches the concept *and* the depth in-house, on the Lantern Home thread. No outsourced teaching: an external resource can explain "Python strings" generically, but it can't teach them *as finding the order number in one of Lantern's customer messages*, and that situated coherence (same client, same files, same vocabulary across 104 units) is a pedagogical asset curation can't buy. A chapter builds the thing in front of the reader a few explained lines at a time, asks before it shows, lets a real mistake happen and reads the error together, and names a term only after showing the thing. It is plain Markdown with no template: there is no fixed section list, no block vocabulary, and no reading-grade target, because every one of those produced prose written to the rule rather than to the reader (the record is `docs/lesson-design.md` §7). The chapter *is* the worked example; there is no parallel task and no second company.
- **The assignment** (`assignment.md`) continues the chapter in the author's own words: the chapter's program on the client's real files with the last steps missing (backward fading), what the checks look for and why, and where people get stuck with the fix under each heading. For code units the student completes `starter/` against `checks.yaml` (Layer 1, auto-graded, each check's message naming the missing step in plain words); for conceptual units the student writes against a rubric (Layer 2). Deliverables run against a one-click sandboxed environment with the student's own Lantern data variant where practical, so "it doesn't run on my machine" stops being a category of stuckness.
- **Recall** is a set of questions per unit (`practice.retrieval_seeds`, questions with reference answers, never facts to recognise) that the platform asks from memory after the chapter and again days later, graded against the chapter text. This is spaced retrieval, the highest-utility review technique in the literature, and it feeds the "can explain every why" graduation bar. Facts are never planted in the prose for a drill to find.
- **The platform's apparatus** follows the assignment on the page, each card labelled by what it is and carrying no bridging copy: the practice workbench and submission, the automated checks, the rubric where one applies, the recall drill, and the tutor chat. The stuck-help lives in the assignment prose; the tutor reads the same `unstuck` entries server-side.

**Verified, not linted.** The one mechanical gate a chapter passes is `content/tools/run-lesson-code.py`: every Python block on the page is executed in a scratch copy of the client's files and its real output is compared with what the page shows, tracebacks included. Everything else about a chapter is judged by a **first reader** (`.agents/agents/first_reader/agent.md`) reading it cold as the student it is written for and reporting, with line numbers, every place a student would stop. The division of labour: concepts and depth are school-authored chapters (one-time capital cost), method transfer happens inside the chapter and its faded assignment (one-time, doesn't scale with students), infinite practice and 11pm questions are the tutor's teach mode (marginal LLM cost), and proof of competence stays with the verification engine (§3).

**Living-document content ops:** the curriculum itself flags that its own tool stack (LangGraph, Langfuse, Qdrant, Unsloth) will drift. Each unit has a scheduled quarterly audit (search-assisted, human-signed-off) that checks named tools/versions are still current and swaps them if not. This is a content-ops task, not a support task — it happens on a schedule, not in response to a ticket.
...
*An example of a rubric — grading 5.1.1 ("model correctly chooses between tools it hasn't seen examples of before"):*
```
Rubric:
...
- Every student gets their own variant of Lantern Home's synthetic data — different message volumes, different rulebook clauses, different numbers. Copying a peer's repo is pointless before review (Layer 3 catches outsourcing *at* review time; the variants kill it beforehand), and each capstone ends up a genuinely individual portfolio piece.
...
- **Public build gallery**, opt-in: shipped Lantern-style systems and portfolio projects, visible across cohorts. Functions simultaneously as motivation (peer visibility), marketing (prospective students see real output), and a soft integrity check (public work invites public scrutiny).
...
- **Submission system** — GitHub OAuth to auto-pull repos and trigger CI (Layer 1); file/recording upload for the rest; a sandbox provisioner that spins up per-student environments and each student's Lantern Home data variant.
...
- **Progress graph** — phase/module/sub-module tree with gate logic, feeding the student dashboard and the weekly digest, and rendering the Lantern Home pipeline as a growing map so every card visibly shows where it plugs into the running system.
...
- **1–2 curriculum author/maintainers** — own lesson quality, assignments, freshness audits, and rubrics. Authoring is the heavy lift at launch; at steady state this is a maintenance role, which is why the count stays low.
...
5. **Open the capstone bar publicly** (Section 14, verified) as the core marketing asset — real students' real Lantern-style systems are stronger proof than anything the school could dispute about itself.
